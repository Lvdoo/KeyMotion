...
class FingerDetection : 
    ...
    def get_finger_data(self, result, width, height) :
        """
    ...
    def get_movement(self, finger_pos) :
        """
        ...
        if  finger_pos["thumb_x"] is None or finger_pos["thumb_y"] is None or last_y["last_thumb_y"] is None :
            self.movement_y["movement_thumb"] = 0
        else :
            self.movement_y["movement_thumb"] = finger_pos["thumb_y"] - last_y["last_thumb_y"]
            if abs(self.movement_y["movement_thumb"]) < 3 :
                self.movement_y["movement_thumb"] = 0


        if  finger_pos["index_x"] is None or finger_pos["index_y"] is None or last_y["last_index_y"] is None :
            self.movement_y["movement_index"] = 0
        else :
            self.movement_y["movement_index"] = finger_pos["index_y"] - last_y["last_index_y"]
            if abs(self.movement_y["movement_index"]) < 3 :
                self.movement_y["movement_index"] = 0


        if  finger_pos["middle_x"] is None or finger_pos["middle_y"] is None or last_y["last_middle_y"] is None :
            self.movement_y["movement_middle"] = 0
        else :
            self.movement_y["movement_middle"] = finger_pos["middle_y"] - last_y["last_middle_y"]
            if abs(self.movement_y["movement_middle"]) < 3 :
                self.movement_y["movement_middle"] = 0


        if  finger_pos["ring_x"] is None or finger_pos["ring_y"] is None or last_y["last_ring_y"] is None :
            self.movement_y["movement_ring"] = 0
        else :
            self.movement_y["movement_ring"] = finger_pos["ring_y"] - last_y["last_ring_y"]
            if abs(self.movement_y["movement_ring"]) < 3 :
                self.movement_y["movement_ring"] = 0


        if  finger_pos["pinky_x"] is None or finger_pos["pinky_y"] is None or last_y["last_pinky_y"] is None :
            self.movement_y["movement_pinky"] = 0
        else :
            self.movement_y["movement_pinky"] = finger_pos["pinky_y"] - last_y["last_pinky_y"]
            if abs(self.movement_y["movement_pinky"]) < 3 :
                self.movement_y["movement_pinky"] = 0

        return self.movement_y


    def update_last_y(self, finger_pos):
        """
        ...
        self.last_y["last_thumb_y"] = finger_pos["thumb_y"]
        self.last_y["last_index_y"] = finger_pos["index_y"]
        self.last_y["last_middle_y"] = finger_pos["middle_y"]
        self.last_y["last_ring_y"] = finger_pos["ring_y"]
        self.last_y["last_pinky_y"] = finger_pos["pinky_y"]
